(function () {
  ...
  function drivers(list = data.drivers, favoriteView = false) { return `<section class="page-shell">${heading(favoriteView ? 'YOUR' : 'THE', favoriteView ? 'FAVORITES' : 'DRIVERS', favoriteView ? 'The drivers you are following, saved on this device.' : 'Twenty drivers. Ten teams. One grid.')} ${list.length ? `<div class="driver-grid driver-grid--all">${list.map(d => driverCard(d, data.drivers.indexOf(d)+1)).join('')}</div>` : `<div class="empty-state"><span>☆</span><h2>No favorites yet</h2><p>Tap the star on any driver card to build your grid.</p><a class="button" href="#drivers">Browse drivers</a></div>`}</section>`; }
